...
def Page():
    ...
    tempo_data = RemoteGeoData_ArcGISImageServer(
        "https://gis.earthdata.nasa.gov/image/rest/services/C2930763263-LARC_CLOUD/", name='TEMPO')
    ...
    glue_app.add_link(glue_app.data_collection["Power_Plants"], 'Longitude', glue_app.data_collection["TEMPO"],
                    'Pixel Axis 0')
    glue_app.add_link(glue_app.data_collection["Power_Plants"], 'Latitude', glue_app.data_collection["TEMPO"],
                    'TEMPO_NO2_L3_V03_HOURLY_TROPOSPHERIC_VERTICAL_COLUMN_BETA')
    ...
    map_viewer = cast(IPyLeafletMapViewer, glue_app.new_data_viewer("map", data=tempo_data, state=map_state,
                                                                    show=False))
