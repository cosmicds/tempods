...
from ipywidgets import SelectionSlider, DatePicker, widgets, FloatSlider, DOMWidget
from ipywidgets import Layout as WidgetLayout
import pandas as pd
import numpy as np
...
def Page():
    ...
    def _setup_mapviewer(glue_app: gj.JupyterApplication) -> IPyLeafletMapViewer:
        coastlines_path = Path(__file__).parent.parent.parent.parent / "notebooks" / "coastlines.geojson"
    ...
    def _setup_power_data(glue_app: gj.JupyterApplication) -> Data:
        if "Power_Plants" not in glue_app.data_collection:
            powerplant_data = Path(__file__).parent.parent.parent.parent / "notebooks" / "Power_Plants.csv"
    ...
    def _setup_slider():
        ...
        time_strings = [convert_from_milliseconds(t) for t in time_values]
        time_options = [(time_strings[i], time_values[i]) for i in range(len(time_values))]
        return SelectionSlider(description='Time (UTC):', options=time_options, layout=WidgetLayout(width='700px', height='25px'))
